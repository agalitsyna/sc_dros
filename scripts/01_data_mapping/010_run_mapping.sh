...
do
  ...
  pairtools parse ${BAMDIR}/${pref}.bam -c $GENOMEDIR/${GENOME}.reduced.chrom.sizes --output ${PAIRDIR}/${pref}.pair -f $GENOMEDIR/rfrags_${GENOME}_${RENZYME}.txt --walks-policy ligation_junctions --drop-sam --add-columns rfrag --output-stats ${STATSDIR}/${pref}.txt
  ...
  then
    ...
    pairtools parse ${BAMDIR}/${pref}.bam -c $GENOMEDIR/${GENOME}.reduced.chrom.sizes --output ${PAIRDIR}/${pref}.pair.full -f $GENOMEDIR/rfrags_${GENOME}_${RENZYME}.txt --walks-policy mask --drop-sam --output-stats ${STATSDIR}/${pref}.txt.full
    ...
    #pairtools restrict ${PAIRDIR}/${pref}.pair.full --frags $GENOMEDIR/rfrags_${GENOME}_${RENZYME}.txt --output ${PAIRDIR}/${pref}.pair.full.restricted
  fi
