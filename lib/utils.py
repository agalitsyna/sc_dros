"""
TODO: rename basic to something more appropriate...
...
#### New boundaries intersection function

def extend_boundaries(vect, offset=1):
...
def compare_segmentations(args):
    seg1, seg2, offset, state = args

    v1 = np.unique(seg1.ravel())
    v2 = np.unique(seg2.ravel())

    if state>=0:
        v = extend_boundaries(np.unique(shuffle_segmentation(seg2, seed=state).ravel()), offset=offset)
    else:
        v = extend_boundaries(v2, offset=offset)

    return np.sum(np.in1d(v1, v)), len(v1), len(v2)

def compute_similarity_segmentations(seg_dct1, seg_dct2, chrms=None, niter=1000, offset=1, nproc=5):

    if chrms is None:
        chrms = seg_dct1.keys()

    resulting_observed = [0, 0, 0]
    resulting_bg = [[0, 0, 0] for x in range(niter)]
    
    for ch in chrms:
        segmentation1 = seg_dct1[ch]
        segmentation2 = seg_dct2[ch]

        observed = compare_segmentations( (segmentation1, segmentation2, offset, -1) )

        random_states = np.random.randint(0, 1e6, niter)

        if nproc > 1:
            args = [(segmentation1, segmentation2, offset, state) for state in random_states]
            from multiprocessing import Pool
            with Pool(nproc) as p:
                bg = p.map(compare_segmentations, args)

        else:
            bg = []
            for state in random_states:
                bg += [ compare_segmentations( (segmentation1, segmentation2, offset, state) ) ]

        resulting_observed = [sum(x) for x in zip(resulting_observed, observed)]
        resulting_bg = [[sum(x) for x in zip(resulting_bg[i], bg[i])] for i in range(niter)]
        
    return(resulting_observed, resulting_bg)

def compute_pvalues(val, background, interpolate=True):
    if interpolate:
        mu, std = scipy.stats.norm.fit(background)
        pv = 1 - scipy.stats.norm.cdf((val - mu) / std)
    else:
        pv = (100 - scipy.stats.percentileofscore(background, val)) / 100
    return pv


### Old boundaries intersection function
